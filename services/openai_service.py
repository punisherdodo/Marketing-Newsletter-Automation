...
class OpenAIService:
    ...
    def _parse_json(self, raw: str) -> dict:
        ...
        try:
            ...
            return {
                ...
                "raw_output": raw[:500],
                "suggested_next_step": "Try re-running the generation. If the issue persists, the model may have returned malformed output.",
            }
    ...
    def generate_campaign_content(self, topic: str, campaign_name: str = "",
        ...
        channels_str = ", ".join(channels) if channels else "Email, Newsletter"
        context_parts = []
        if campaign_name:
        ...
        prompt = f"""You are writing marketing content for NovaMind, an AI startup that helps small creative agencies automate daily workflows.
...
- "blog_draft": string (400-600 words, practical and grounded in agency workflow problems)
- "newsletters": object with keys "Agency Founder", "Operations Manager", "Creative Lead", "Account / Client Services Lead", "Strategy / Marketing Lead" — each value a 220-300 word newsletter
- "subject_lines": object with same persona keys, each a compelling subject line
- "ab_test_ideas": array of 2 A/B test suggestions
- "persona_rationale": object with same persona keys, each a 1-sentence rationale

Newsletter rules:
- Each newsletter is a cohesive mini-essay covering: core pain point, how AI helps, why it matters, one takeaway
- Tone should be practical, specific, human
...
- "subject_line_tests": object with persona keys
- "newsletter_improvements": object with persona keys

Base recommendations on the data. Be practical. Return valid JSON only."""
        try:
            raw = self._call(prompt)
            return self._parse_json(raw)
        except Exception as e:
...
def _safe_error(e: Exception) -> str:
    msg = str(e)
    keywords = ["key", "token", "secret", "auth", "bearer", "sk-"]
    for kw in keywords:
        if kw in msg.lower():
    ...
    error_map = {
        ...
        "InvalidRequestError": "Invalid request sent to OpenAI. Try a different model name.",
        "openai": "OpenAI request failed. Check your key and try again.",
    }
    for key, friendly in error_map.items():
        if key in type(e).__name__ or key in msg:
            return friendly
